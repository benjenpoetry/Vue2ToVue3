...
export interface JBooleanLiteral {
    type: 'BooleanLiteral';
    value: string
    /** $ childType $ **/
}

export interface JBooleanLiteralVirtual {
    type: 'BooleanLiteral';
    value?: string
    /** $ childVirtualType $ **/
}

export function _JBooleanLiteral (
    mapping: JNodeMapping,
    parentName: JAstTypeKey,
    value: string,
    callback: () => void
) {
    const [child, index, children] = getContextWithJNodeMapping<JBooleanLiteralVirtual>(mapping, 'BooleanLiteral');
    child.value = value;
